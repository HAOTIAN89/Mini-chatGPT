...
parser.add_argument('--ppo_epochs', type=int, default=256000, help='Number of PPO epochs')
...
def ppo_iter(mini_batch_size, new_log_probs, old_log_probs, advantage):
    ...
    for i in range(batch_size // mini_batch_size):
        yield\
            new_log_probs[ids[i * mini_batch_size:(i + 1) * mini_batch_size]],\
            old_log_probs[ids[i * mini_batch_size:(i + 1) * mini_batch_size]],\
            advantage[ids[i * mini_batch_size:(i + 1) * mini_batch_size]]


def ppo_update(ppo_epochs, mini_batch_size, ppo_log_probs, sft_log_probs, advantages, clip_param=args.ppo_clip_ratio):
    for _ in range(ppo_epochs):
        for new_log_probs, old_log_probs, advantage in ppo_iter(mini_batch_size, ppo_log_probs, sft_log_probs, advantages):
            optimizer.zero_grad()
...
for epoch in tqdm(range(args.ppo_epochs)):
    for instructions in train_dataloader:
        ...
        instructions = [i[0] for i in instructions_interactions]
        ...
        ppo_log_probs = [get_log_prob(ppo_model, i[0], i[1]) for i in instructions_interactions]
        with torch.no_grad():
            sft_log_probs = [get_log_prob(sft_model, i[0], i[1]) for i in instructions_interactions]
            
        # Convert log probabilities to tensors
        ppo_log_probs = torch.stack(ppo_log_probs)
        sft_log_probs = torch.stack(sft_log_probs)

        # Update the model
        ppo_update(args.ppo_epochs, args.mini_batch_size, ppo_log_probs, sft_log_probs, advantages)
